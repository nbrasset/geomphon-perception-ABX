...
rm(list=ls())
library(dplyr)
data<-read.csv("/Users/post-doc/Desktop/geomphon_pilot_results_for_analysis.csv")

#group by tripletid, which is the stimulus, then summarise how many correct answers for that triplet
grouped<- dplyr::group_by(data,tripletid,log_delta_dist_div) %>%
...
plot(grouped$log_delta_dist_div, grouped$accuracy)
